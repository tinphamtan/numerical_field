...
public:
	...
	double CouplingCoefficient_plus(const SmallSquare& aSq);//calculate the coupling coefficient with circle aprox
	bool IsSelfCoupling(const SmallSquare & aSq);
	bool IsOnTopOfTheOther(const SmallSquare & aSq);
	double SelfCoupling();
	double DistanceCoupling(const SmallSquare & aSq);
	double CircleApprx(const SmallSquare & aSq);
